...
use crate::{fsm::EventContext, FsmBackend};
...
impl<'a, TFsm, TContext, TEvent, TState> FsmEventBuilderState<'a, TFsm, TContext, TEvent, TState> {
    ...
    pub fn internal_transition(
        &self,
    ) -> FsmEventBuilderTransition<'_, TFsm, TContext, TEvent, TState> {
    ...
    pub fn self_transition(&self) -> FsmEventBuilderTransition<'_, TFsm, TContext, TEvent, TState> {
        FsmEventBuilderTransition {
            _state_event_builder: self,
        }
    }
    ...
    pub fn transition_to<TStateTo>(
        &self,
    ) -> FsmEventBuilderTransitionFull<'_, TFsm, TContext, TEvent, TState, TStateTo> {
...
impl<'a, TFsm, TContext, TEvent, TState>
    FsmEventBuilderTransition<'a, TFsm, TContext, TEvent, TState>
where
    TFsm: FsmBackend,
{
    /// An action that happens when the currently active state receives this event. No transitions.
    pub fn action<
        TAction: Fn(&TEvent, &mut EventContext<'a, TFsm, FsmQueueMock<TFsm>>, &mut TState),
    >(
        &mut self,
        _action: TAction,
    ) -> &mut Self {
    ...
    pub fn guard<
        TGuard: Fn(
            &TEvent,
            &EventContext<'a, TFsm, FsmQueueMock<TFsm>>,
            &<TFsm as FsmBackend>::States,
        ) -> bool,
    >(
        &mut self,
        _guard: TGuard,
...
impl<'a, TFsm, TContext, TEvent, TStateFrom, TStateTo>
    FsmEventBuilderTransitionFull<'a, TFsm, TContext, TEvent, TStateFrom, TStateTo>
where
    TFsm: FsmBackend,
{
    ...
    pub fn action<
        TAction: Fn(
            &TEvent,
            &mut EventContext<'a, TFsm, FsmQueueMock<TFsm>>,
            &mut TStateFrom,
            &mut TStateTo,
        ),
    >(
