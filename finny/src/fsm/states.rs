use crate::{lib::*, FsmBackend};

use crate::FsmResult;

/// The implementation should hold all of the FSM's states as fields.
pub trait FsmStates<TFsm>: FsmStateFactory<TFsm>
where
    TFsm: FsmBackend,
{
    ...
    type CurrentState: Clone
        + Copy
        + Debug
        + Default
        + AsRef<[FsmCurrentState<Self::StateKind>]>
        + AsMut<[FsmCurrentState<Self::StateKind>]>
        + 'static;
}
    ...
    S: Clone + Copy,
{
    pub fn all_stopped(current_states: &[Self]) -> bool {
        current_states
            .iter()
            .all(|s| matches!(s, FsmCurrentState::Stopped))
    }
}

impl<S> Debug for FsmCurrentState<S>
where
    S: Debug + Copy,
{
...
impl<S> Default for FsmCurrentState<S>
where
    S: Clone + Copy,
{
    fn default() -> Self {
...
pub trait FsmStateFactory<TFsm>
where
    Self: Sized,
    TFsm: FsmBackend,
{
...
impl<TState, TFsm> FsmStateFactory<TFsm> for TState
where
    TState: Default,
    TFsm: FsmBackend,
{
